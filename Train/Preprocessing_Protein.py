...
def main():
    ...
    save_mc_path = os.path.join(protein_preprocess_dir, "meta_cell_2_2")
    ...
    print("Protein section names:")
    for x in protein_section_names:
        print("  ", x)

    # =========================================================
    # 4. Build mapping from Protein section name to master section name
    # =========================================================
    ...
    protein_to_master_name = {}
    unmatched_protein_sections = []

    for protein_name in protein_section_names:
        if protein_name in master_section_set:
            protein_to_master_name[protein_name] = protein_name
        else:
            unmatched_protein_sections.append(protein_name)

    print("\nProtein to master section mapping:")
    for k, v in protein_to_master_name.items():
        print(f"  {k}  -->  {v}")

    if len(unmatched_protein_sections) > 0:
        print("\n[Warning] These Protein sections could not be matched to HE master order:")
        for x in unmatched_protein_sections:
            print("  ", x)
    ...
    # =========================================================
    print("\n" + "=" * 80)
    print("Step 1. Optional metacell construction for Protein")
    print("=" * 80)

    if run_metacell:
        print("Metacell construction is ENABLED.")

        for i, (path, section_name) in enumerate(zip(protein_paths, protein_section_names), start=1):
            print(f"\n[{i}/{len(protein_paths)}] Processing Protein section:")
            print(path)
            ...
            adata_protein = sc.read_h5ad(path)
            print(f"  Loaded Protein AnnData with shape: {adata_protein.shape}")

            if "UNI_feature" in adata_protein.obsm:
                del adata_protein.obsm["UNI_feature"]
                print("  Removed obsm['UNI_feature']")

            adata_protein.var_names_make_unique()

            adata_protein_mc = metacell_construction_optimized(adata_protein)

            meta_map = adata_protein_mc.uns["meta_to_original"]
            ...
            del adata_protein_mc.uns["meta_to_original"]

            adata_protein_mc.var_names_make_unique()

            mc_h5ad_path = os.path.join(save_mc_path, f"{section_name}_protein_mc.h5ad")
            adata_protein_mc.write(mc_h5ad_path)
            print(f"  Saved metacell h5ad to: {mc_h5ad_path}")
            print(f"  Metacell shape = {adata_protein_mc.shape}")

            del adata_protein, adata_protein_mc, meta_map
            gc.collect()

        print("\n[Done] Protein metacell construction completed.")

    else:
        print("Metacell construction is DISABLED. Skipping this step.")

    # =========================================================
    # 6. Prepare Protein data_dict in master order
    # =========================================================
    print("\n" + "=" * 80)
    print("Step 2. Prepare Protein data_dict for load_data")
    print("=" * 80)

    protein_dict_by_master_section = {}

    if run_metacell:
        print("Using metacell Protein for preprocessing.")

        for protein_name in protein_section_names:
            if protein_name not in protein_to_master_name:
                continue

            master_name = protein_to_master_name[protein_name]
            mc_h5ad_path = os.path.join(save_mc_path, f"{protein_name}_protein_mc.h5ad")

            if os.path.exists(mc_h5ad_path):
                print(f"Loading metacell Protein: {mc_h5ad_path}")
                adata_tmp = sc.read_h5ad(mc_h5ad_path)
                adata_tmp.var_names_make_unique()
                protein_dict_by_master_section[master_name] = adata_tmp
            else:
                print(f"[Warning] Missing metacell Protein file: {mc_h5ad_path}")

    else:
        print("Using original Protein for preprocessing.")

        for path, protein_name in zip(protein_paths, protein_section_names):
            if protein_name not in protein_to_master_name:
                continue

            if not os.path.exists(path):
                print(f"[Warning] Missing original Protein file: {path}")
                continue
    ...
    for section_name in all_section_names:
        if section_name in protein_dict_by_master_section:
            adata_protein_for_preprocessing.append(protein_dict_by_master_section[section_name])
        else:
            adata_protein_for_preprocessing.append(None)
            missing_sections.append(section_name)

    print(
        f"\nValid Protein sections: "
        f"{sum(x is not None for x in adata_protein_for_preprocessing)} / {len(all_section_names)}"
    )
    print(f"Missing Protein sections automatically filled with None: {len(missing_sections)}")
    ...
    data_dict = {
        "Protein": adata_protein_for_preprocessing
    }
    ...
    # =========================================================
    print("\n" + "=" * 80)
    print("Step 3. Run load_data preprocessing for Protein")
    print("=" * 80)
    ...
    # =========================================================
    print("\n" + "=" * 80)
    print("Step 4. Save Protein preprocessing outputs")
    print("=" * 80)
